...
function ConstantFluxStokesDrift(grid, water_kinematic_momentum_flux, peak_wavenumber;
    ...
    water_kinematic_momentum_flux = water_kinematic_momentum_flux <= 0 ? water_kinematic_momentum_flux : - water_kinematic_momentum_flux
...
function compute_stokes_drift!(t, cfsd::ConstantFluxStokesDrift, kᵖ=peak_wavenumber(t, cfsd))
    ...
    @inbounds cfsd.∂z_uˢ[1, 1, k_surface] = Cᵝ * a★ * (kⁿ - kᵖ) + 4Cᴮ * √(2g) * (kⁱ - kⁿ)
...
function write_stokes_drift!()
    grid = RectilinearGrid(size = 128, z = (-256, 0), topology = (Flat, Flat, Bounded))

    momentum_flux = parse(Float64, get(ENV, "momentum_flux", "0.0")) / 1027

    if momentum_flux == 0
        fpaths = ["./usp_file.dat", "./usprof.dat", "./usp.dat", "./usprof_file.dat"]
        for fpath in fpaths
            open(fpath, "w") do file
               write(file, "2000-01-01 00:00:00  100  1\n")
                for k in grid.Nz : -1 : 1
                    zC = znode(1, 1, k, grid, Center(), Center(), Center())
                    write(file, @sprintf("%.3e %.3e %.3e\n", zC, 0.0, 0.0))
                end
            end
        end
    else
        stokes_drift_peak_wavenumber = 1e-6 * 9.81 / abs(momentum_flux)
        stokes_drift = ConstantFluxStokesDrift(grid, momentum_flux, stokes_drift_peak_wavenumber)
        uˢ₀ = stokes_drift.uˢ[1, 1, grid.Nz]
        ...
        for fpath in fpaths
        open(fpath, "w") do file
            write(file, "2000-01-01 00:00:00  100 3\n")
            for k in 1:grid.Nz
                zC = znode(1, 1, k, grid, Center(), Center(), Center())
                write(file, @sprintf("%.3e   %.3e   %.3e\n", zC, stokes_drift.uˢ[1, 1, k], 0.0))
            end
        end 
        end
    end
